...
//        JSONArray jsonArray = orderCurveJSONTest(null);
    ...
    public Object getCompareCurve(@RequestBody List<CurveParam> curveParamList, HttpServletRequest request) {
        ...
        JSONArray jsonArrays = new JSONArray();
				for (CurveParam curveParam : curveParamList) {
					...
					JSONArray jsonArray = orderCurveJSONOne(runningDataHistoryList, type);
					JSONObject jsonObject = new JSONObject();
					jsonObject.put("data", jsonArray);
    ...
    private JSONArray orderCurveJSON(List<RunningDataTotalDO> runningDataHistoryList) {
        ...
        for (RunningDataTotalDO data : runningDataHistoryList) {
            ...
            jsonHouseObject.put("openStatus", convertToNum(data.getOpenStatus()));
            jsonHouseObject.put("stepTotal", data.getStepTotal());
            jsonHouseObject.put("executionStep", data.getExecutionStep());
            jsonArray.add(jsonHouseObject);
        }
        return jsonArray;
    }
    
    private JSONArray orderCurveJSONOne(List<RunningDataTotalDO> runningDataHistoryList, String type) {
      RunningDataTotalDO total = new RunningDataTotalDO();
      JSONArray jsonArray = new JSONArray();
      if (runningDataHistoryList.size() == 0) {
          return jsonArray;
      ...
      for (RunningDataTotalDO data : runningDataHistoryList) {
          JSONObject jsonHouseObject = new JSONObject();  
          jsonHouseObject.put("value", Tools.getObjValue(data, type));
          jsonHouseObject.put("time", DateUtil.toDateHourString(data.getSystemReadTime().getTime()));
          jsonArray.add(jsonHouseObject);
      }
      return jsonArray;
    ...
    private  Integer convertToNum(String openStatus) {
    	if(openStatus.contains("开")){
    		return 1;
    	}else if(openStatus.contains("关")){
    		return 0;
    	}
			return openStatus.charAt(openStatus.length() -1) - 48;
		}
