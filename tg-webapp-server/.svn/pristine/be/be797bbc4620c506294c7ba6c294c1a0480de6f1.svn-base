...
public final class StringUtil {
    ...
    public static final Pattern pattern = Pattern.compile("(IO|[A-Z])");;
    ...
    public static String toFormatString(String strValue, int strLength, boolean formatFlg,
	    String formtaValue) {
	...
	if (strValue != null && strValue.getBytes().length > strLength) {
		...
		if ((newStrvalue + strValue.toCharArray()[i]).toString()
			.getBytes().length > strLength) {
    ...
    public static byte[] bcdString2Bytes(int param) {
     ...
     * @return
     */
    public static int hexToDbc(byte[] src) {
	return hexToDbc(src, ByteOrder.BIG_ENDIAN);
    }
     ...
     * @return
     */
    public static int hexToDbc(byte[] src, ByteOrder endian) {
	int result;
