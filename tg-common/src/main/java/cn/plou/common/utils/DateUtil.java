...
public final class DateUtil {
	...
	public static String getNowHourMinite() {
		DateFormat pattern = new SimpleDateFormat("HH:mm");
		return pattern.format(getSysdate());
	}

	public static String getNowTime() {
		DateFormat pattern = new SimpleDateFormat(DEFAULT_TIME_FORMAT);
		return pattern.format(getSysdate());
	}
	...
	public static Date addSecond(Date date, int addition) {
		return addTime(date, Calendar.SECOND, addition);
	}
	...
	public static boolean isBeforeEqualsDate(Date preDatetime, Date afterDatetime) {
		boolean result = false;
		...
		if (!preDatetime.after(afterDatetime)) {
			result = true;
		} 
		return result;
	}
	...
	public static int calculateMonth(Date date1, Date date2) {
		...
		int month = (cal1.get(Calendar.YEAR) - cal2.get(Calendar.YEAR)) * 12 + cal1.get(Calendar.MONTH) - cal2.get(Calendar.MONTH);
		return month;
	 ...
	 * @return
	 */
	public static int minitesBetween(Date smdate, Date bdate) {
		try {
			...
			cal.setTime(bdate);
			long time2 = cal.getTimeInMillis();
			long between_minite= (time2 - time1) / (1000 * 60);

			return Integer.parseInt(String.valueOf(between_minite));
		} catch (Exception e) {
